...
function ElementList({ elementSetId, elements, currentElement, onDelete, getElementUrl }) {
  const classes = useStyles();
    ...
    return (
      <ListItem key={el.id}
        ...
        <ListItemSecondaryAction>
          <IconButton aria-label="Удалить" onClick={e => onDelete(el)}>
            <DeleteIcon />
          </IconButton>
        </ListItemSecondaryAction>
      </ListItem>
    ...
    listItems = pairList.map(({ first, second, result }, i) => {
      ...
      return (
        <ListItem key={i}>
          ...
          {onDelete ?
            <ListItemSecondaryAction>
              <IconButton aria-label="Удалить" onClick={e => onDelete(i)}>
                <DeleteIcon />
              </IconButton>
            </ListItemSecondaryAction> :
            undefined}
...
function ElementMadeOfEditor({ element, elements, onChange, getElementUrl }) {
  ...
  const listItems = madeOf.map(({ first, second }, i) => {
    const firstEl = elements.find(e => e.id == first.id);
    const secondEl = elements.find(e => e.id == second.id);
    return (
      <ListItem key={i}>
        <ListItemText
          // primary={`${firstEl.name} + ${secondEl.name}`} 
          primary={
            <div>
              <Link to={getElementUrl(firstEl.id)}>{firstEl.name}</Link>{' + '}
              <Link to={getElementUrl(secondEl.id)}>{secondEl.name}</Link>
            </div>
          }
        />
        <ListItemSecondaryAction>
          <IconButton aria-label="Удалить" onClick={e => deleteRow(i)}>
            <DeleteIcon />
          </IconButton>
        </ListItemSecondaryAction>
      </ListItem>
        ...
        return (
          ...
          >
            <div style={{ "textAlign": "right" }}>
              {element.icon ?
                <img src={element.icon} className={classes.elementIcon} /> :
...
function ElementSetEditorInner({ match, history }) {
  ...
  const playableLink = `${location.origin}/games/${elementSetId}`
